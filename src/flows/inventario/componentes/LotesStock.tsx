...
export function LotesStock() {
  ...
  return <section className="stock-lotes">
    <div className="stock-lotes-filtros">
      <label className="stock-lotes-busqueda"><div><Icon name="search" size={17} /><input aria-label="Buscar ítem, marca, lote o almacén" placeholder="Buscar ítem, marca, lote o almacén..." value={busqueda} onChange={(event) => setBusqueda(event.target.value)} /></div></label>
      <label><select aria-label="Filtrar por sucursal" value={sucursal} onChange={(event) => { setSucursal(event.target.value); setAlmacen('todos'); }}><option value="todas">Todas las sucursales</option><option value="hospital-maria-esperanza">Hospital María Esperanza</option></select></label>
      <label><select aria-label="Filtrar por almacén" value={almacen} onChange={(event) => setAlmacen(event.target.value)}><option value="todos">Todos los almacenes</option>{almacenes.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
    </div>
