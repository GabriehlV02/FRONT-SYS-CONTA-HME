...
export function LotesStock() {
  ...
  return <section className="stock-lotes"><h2>Stock por marca y lote</h2><p>Orden de salida FIFO: primero la fecha de recepción más antigua dentro del almacén seleccionado. Los lotes vencidos no se venden. Cambiar el precio no modifica este orden.</p>
    <label>Buscar ítem, marca o almacén <input value={busqueda} onChange={e => setBusqueda(e.target.value)} /></label>
    {error && <p role="alert">{error}</p>}
    <div className="stock-lotes-tabla"><table><thead><tr>{['Ítem', 'Marca', 'Lote', 'Recepción', 'Almacén', 'Disponible', 'Costo/unidad', 'Vence'].map(c => <th key={c}>{c}</th>)}</tr></thead><tbody>
